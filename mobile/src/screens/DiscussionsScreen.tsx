...
// non-personal content — general topic chat, shared news reactions, and
// standalone how-to/explainer write-ups — same OR-within-field facet
// mechanism NewsScreen uses for doc_kind:gov_news, just on a different
// field (api.py's _facets_filter). Reuses searchPostings/PostingCard as-is:
// zero backend changes needed.
const DISCUSSIONS_FACETS = ['tags:discussion', 'tags:blog'];
const PAGE_SIZE = 20;
...
export function DiscussionsScreen({ navigation }: any) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await searchPostings('', { facets: DISCUSSIONS_FACETS, pageSize: PAGE_SIZE });
      ...
      >
        ...
        {!loading && !error && items.length === 0 && (
          <EmptyState
            ...
            body="Check back soon for shared articles and general immigration topics."
          />
        )}
